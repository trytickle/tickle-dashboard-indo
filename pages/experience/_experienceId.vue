<template>
  <div class="container" style="padding:20px;max-width:800px;">
    <div v-if="isEditMode">
    <div class="container" v-if="isEditMode" style="padding:20px;max-width:640px;margin:auto;">
    <p class="title is-3">Edit Experience</p>
    <p class="subtitle is-5">This will update the submission</p>
    ...
    <div class="field is-grouped" style="padding-bottom:50px;">
      <div class="control">
        <button class="button is-link is-loading" v-if="isLoading">Save Submission</button>
        <button class="button is-link" @click="saveSubmission" v-else>Save Submission</button>
      </div>
      <div class="control">
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      this.whatIProvide = submission.whatIProvide
      this.whereWeBe =  submission.whereWeBe
      this.notes = submission.notes
      this.locationName = submission.whereWeMeet.locationName
      this.aptSuiteBuilding = submission.whereWeMeet.aptSuiteBuilding
      this.streetAddress = submission.whereWeMeet.streetAddress
      this.zipcode = submission.whereWeMeet.zipcode
      this.city = 'Singapore'
    ...
    async saveSubmission() {
      ...
      try {
        const result = await db.collection('submissions').doc(body.submissionId).update(body);
        console.log(result+" result")
        if (this.coverPhotoImage) {
          await this.uploadImage(this.experienceId)
          await db.collection('submissions').doc(this.experienceId).update({
